...
-   **통합 검색 페이지**: 재료명, 레시피명, 조리 순서 등 다양한 기준으로 레시피를 검색하고 결과를 목록으로 확인 가능
-   **실시간 검색 보조**: 검색어 입력 시 관련 단어를 실시간으로 추천하는 자동 완성 기능과 상세 필터 옵션을 제공
-   **레시피 상세 정보**: 각 레시피의 필요한 재료와 조리 방법을 상세하게 제공
-   **반응형 디자인**: 다양한 디바이스 환경을 고려한 Radix UI 및 Tailwind CSS 기반의 반응형 인터페이스를 구현
...
export default function SearchListView() {
  // DI 컨테이너로부터 레포지토리 인스턴스를 해석(Resolve)합니다.
...
- 전역 상태 관리는 `Zustand`를 사용하며, 검색어나 필터 설정과 같은 **UI 상태** 관리에 집중합니다. 
- 서버 데이터 페칭 로직은 레포지토리가 담당하도록 설계하여, 스토어(Store)가 비대해지는 것을 방지하고 도메인 로직의 오염을 막았습니다.
...
-   **HTTP Client**: `axios`
-   **Styling**: `Tailwind CSS`, `Radix UI` (접근성 준수를 위한 Headless UI 컴포넌트)
